        ...
        struct wined3d_swapchain_desc *swapchain_desc)
{
    ...
    if (!window || window == GetDesktopWindow())
    {
        ...
        wc.lpszClassName = DDRAW_WINDOW_CLASS_NAME;
      	...
      	window = CreateWindowA(DDRAW_WINDOW_CLASS_NAME, "Hidden D3D Window",
                WS_DISABLED, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN),
                NULL, NULL, getDDHInst(), NULL);
        if (!window)
        {
    ...
    ddraw->decls =  calloc(1, sizeof(*ddraw->decls) * ddraw->declArraySize);
    if (!ddraw->decls)
    {
        ...
        DDSURFACEDESC2 *DDSD, void *Context, LPDDENUMMODESCALLBACK2 cb)
{
    ...
    enum_modes = malloc(sizeof(*enum_modes) * enum_mode_array_size);
    ...
    for(fmt = 0; fmt < (sizeof(checkFormatList) / sizeof(checkFormatList[0])); fmt++)
    {
                ...
                WINED3D_SCANLINE_ORDERING_UNKNOWN, modenum++, &mode) == WINED3D_OK)
        {
            ...
            if (enum_mode_count == enum_mode_array_size)
            {
                ...
                new_enum_modes = realloc(enum_modes, sizeof(*new_enum_modes) * enum_mode_array_size);
                if (!new_enum_modes)
                {
                    free(enum_modes);
                    wined3d_mutex_unlock();
...
struct wined3d_vertex_declaration *ddraw_find_decl(struct ddraw *This, DWORD fvf)
{
    ...
    if(This->declArraySize == This->numConvertedDecls) {
        ...
        convertedDecls = realloc(convertedDecls, sizeof(convertedDecls[0]) * (This->numConvertedDecls + grow));
        if (!convertedDecls)
        {
...
typedef HMONITOR (WINAPI * MonitorFromWindow_func)(HWND  hwnd, DWORD dwFlags);
...
static void CDECL device_parent_mode_changed(struct wined3d_device_parent *device_parent)
{
    ...
    RECT *r;
    HANDLE u32;
    MonitorFromWindow_func MonitorFromWindow_proc = NULL;
    GetMonitorInfoA_func GetMonitorInfoA_proc = NULL;
    ...
    u32 = GetModuleHandle("user32.dll");
    if(u32)
    {
    	GetMonitorInfoA_proc = (GetMonitorInfoA_func)GetProcAddress(u32, "GetMonitorInfoA");
    	MonitorFromWindow_proc = (MonitorFromWindow_func)GetProcAddress(u32, "MonitorFromWindow");
    }

    if(GetMonitorInfoA_proc == NULL || MonitorFromWindow_proc == NULL)
    {
    	/* 95 failback */
    	return;
    }

    monitor = MonitorFromWindow_proc(ddraw->swapchain_window, MONITOR_DEFAULTTOPRIMARY);
    monitor_info.cbSize = sizeof(monitor_info);
    if (!GetMonitorInfoA_proc(monitor, &monitor_info))
    {
        ...
        void **parent, const struct wined3d_parent_ops **parent_ops)
{
    ...
    if (!(ddraw_surface = calloc(1, sizeof(*ddraw_surface))))
    {
