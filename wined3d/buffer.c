...
static void buffer_invalidate_bo_range(struct wined3d_buffer *buffer, UINT offset, UINT size)
{
    ...
    if (buffer->modified_areas >= buffer->maps_size)
    {
        ...
        if (!(new = realloc(buffer->maps, 2 * buffer->maps_size * sizeof(*buffer->maps))))
        {
...
static void buffer_sync_apple(struct wined3d_buffer *This, DWORD flags, const struct wined3d_gl_info *gl_info)
{
    ...
    if(!This->query)
    {
        ...
        This->query = calloc(1, sizeof(*This->query));
        if (!This->query)
        {
...
{
    UINT off = ((UINT)(abs((INT)(x - offset))) % stride);
    if (!off)
        return x;
    if (x > offset)
        return x - off;
        ...
        const struct wined3d_state *state)
{
    ...
    data = malloc(buffer->resource.size);
        ...
        const struct wined3d_sub_resource_data *data, void *parent, const struct wined3d_parent_ops *parent_ops)
{
    ...
    buffer->maps = malloc(sizeof(*buffer->maps));
    if (!buffer->maps)
    {
        ERR("Out of memory\n");
        buffer_unload(&buffer->resource);
...
HRESULT __cdecl wined3d_buffer_create_vb(struct wined3d_device *device, UINT size, DWORD usage, enum wined3d_pool pool,
...
HRESULT __cdecl wined3d_buffer_create_ib(struct wined3d_device *device, UINT size, DWORD usage, enum wined3d_pool pool,
        void *parent, const struct wined3d_parent_ops *parent_ops, struct wined3d_buffer **buffer)
{
    struct wined3d_buffer *object;
    HRESULT hr;
