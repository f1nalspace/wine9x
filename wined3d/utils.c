...
static BOOL init_format_base_info(struct wined3d_gl_info *gl_info)
{
    ...
    gl_info->formats = calloc(1, format_count * sizeof(*gl_info->formats));
    if (!gl_info->formats)
    {
        ...
        const struct wined3d_color *color)
{
    ...
    checkGLcall("Drawing a quad - glUseProgram");
...
static void apply_format_fixups(struct wined3d_adapter *adapter, struct wined3d_gl_info *gl_info)
{
    ...
    if (gl_info->supported[ARB_VERTEX_ARRAY_BGRA])
    {
        ...
        // JHFIX: required ARB_vertex_array_bgra here
...
#define ARG0 0x04
    ...
    if(state->fb->render_targets[0] == NULL)
    {
    	return;
    }
