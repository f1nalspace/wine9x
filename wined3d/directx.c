...
#include <stdint.h>
  ...
  #define DUMMYACCESS1(_var, _uname, _element) _var._uname._element
  #define DUMMYACCESS2(_var, _uname1, _uname2, _element) _var._uname1._uname2._element
#else
	#define DUMMYACCESS1(_var, _uname, _element) _var._element
	#define DUMMYACCESS2(_var, _uname1, _uname2, _element) _var._element
#endif
...
static BOOL wined3d_caps_gl_ctx_create(struct wined3d_adapter *adapter, struct wined3d_caps_gl_ctx *ctx)
{
   ...
   /* WNDCLASS wc      = {0};
    ...
    wc.lpszClassName        = WINED3D_OPENGL_WINDOW_CLASS_NAME;
		...
		if( !RegisterClass(&wc) )
		{
			//printf("RegisterClass failure: %d!\n", GetLastError());
			//goto fail;
			DWORD err = GetLastError();
			if(err != ERROR_CLASS_ALREADY_EXISTS && err != ERROR_SUCCESS && err != ERROR_CALL_NOT_IMPLEMENTED)
			{
				ERR("Re-RegisterClass failure: %d!\n", err);
				goto fail;
			}
		}*/
...
static const struct driver_version_information driver_version_table[] =
{
    ...
    /* Virtualbox/VoodooBox */
    {DRIVER_CHROMIUM,           DRIVER_MODEL_NT5X,  "VBoxDisp.dll",   6, 1,  0},
};
        ...
        const char *gl_vendor_string, const char *gl_renderer)
{
    ...
    if (strstr(gl_vendor_string, "Chronium") || strstr(gl_vendor_string, "Humper"))
        return GL_VENDOR_CHROMIUM;
...
static enum wined3d_pci_vendor wined3d_guess_card_vendor(const char *gl_vendor_string, const char *gl_renderer)
{
    ...
    if (strstr(gl_renderer, "Chromium") || strstr(gl_renderer, "Humper"))
    	  return HW_VENDOR_CHROMIUM;
...
chromium_gl_vendor_table[] =
{
    {GL_VENDOR_CHROMIUM, "Chromium driver",                 cards_chromium,           ARRAY_SIZE(cards_chromium)},
};
...
card_vendor_table[] =
{
    ...
    {HW_VENDOR_INTEL,       "Intel",    intel_gl_vendor_table,
            ...
            card_fallback_intel},
    {HW_VENDOR_CHROMIUM,    "Chromium", chromium_gl_vendor_table,
            sizeof(chromium_gl_vendor_table) / sizeof(*chromium_gl_vendor_table),
            card_fallback_amd},
};
        ...
        DWORD glsl_version, const char *gl_renderer, enum wined3d_gl_vendor *gl_vendor, enum wined3d_pci_vendor *card_vendor)
{
    ...
    if(*card_vendor == HW_VENDOR_SOFTWARE)
    {
    	/* For failback choose nvidia with full dx 9 */
    	//*card_vendor = HW_VENDOR_NVIDIA;
    	//return CARD_NVIDIA_GEFORCE_6800;
    	*card_vendor = HW_VENDOR_VMWARE;
    	wined3d_settings.emulated_textureram = 128*1024*1024;
    	
    	return CARD_VMWARE_SVGA3D;
    }
    
    if(*card_vendor == HW_VENDOR_CHROMIUM)
    {
    	*card_vendor = HW_VENDOR_CHROMIUM;
    	return CARD_CHROMIUM;
    }
...
HMONITOR CDECL wined3d_get_adapter_monitor(const struct wined3d *wined3d, UINT adapter_idx)
{
    ...
    if(u32)
    {
    	MonitorFromPoint_func MonitorFromPoint_proc = (MonitorFromPoint_func)GetProcAddress(u32, "MonitorFromPoint");
    	if(MonitorFromPoint_proc)
    	{
    		return MonitorFromPoint_proc(wined3d->adapters[adapter_idx].monitorPoint, MONITOR_DEFAULTTOPRIMARY);
    	}
    }
 ...
 * nonsenses.
 *
 * Alternative code for 95 is based on this:
        ...
        enum wined3d_format_id format_id, enum wined3d_scanline_ordering scanline_ordering)
{
    ...
    while (EnumDisplaySettingsA95(adapter->DeviceName, j++, &mode))
    {
        if (mode.dmFields & DM_DISPLAYFLAGS)
        {
            if (scanline_ordering == WINED3D_SCANLINE_ORDERING_PROGRESSIVE
                    && (DUMMYACCESS1(mode, u2, dmDisplayFlags) & DM_INTERLACED))
                continue;
        ...
        UINT mode_idx, struct wined3d_display_mode *mode)
{
    ...
    while (i <= mode_idx)
    {
        if (!EnumDisplaySettingsA95(adapter->DeviceName, j++, &m))
        {
        ...
        {
            if (scanline_ordering == WINED3D_SCANLINE_ORDERING_PROGRESSIVE
                    && (DUMMYACCESS1(m, u2, dmDisplayFlags) & DM_INTERLACED))
                continue;
    ...
    else if (DUMMYACCESS1(m, u2, dmDisplayFlags) & DM_INTERLACED)
        ...
        struct wined3d_display_mode *mode, enum wined3d_display_rotation *rotation)
{
    ...
    r = EnumDisplaySettingsA95(adapter->DeviceName, ENUM_CURRENT_SETTINGS, &m); 
    if(!r)
    {
    	WARN("EnumDisplaySettingsA failure: %d\n", GetLastError());
    	memset(&m, 0, sizeof(m));
    	m.dmPelsWidth  = 640;
    	m.dmPelsHeight = 480;
    	m.dmBitsPerPel = 32;
    ...
    if (rotation)
    {
        ...
        *rotation = WINED3D_DISPLAY_ROTATION_0;
        ...
        switch(DUMMYACCESS2(m, u1, s2, dmDisplayOrientation))
        {
            case DMDO_DEFAULT:
                *rotation = WINED3D_DISPLAY_ROTATION_0;
                break;
            ...
            case DMDO_270:
                *rotation = WINED3D_DISPLAY_ROTATION_270;
                break;
            default:
                FIXME("Unhandled display rotation %#x.\n", DUMMYACCESS2(m, u1, s2, dmDisplayOrientation));
...
typedef LONG (WINAPI * ChangeDisplaySettingsExA_func)(LPCSTR lpszDeviceName, DEVMODEA *lpDevMode, HWND hwnd, DWORD dwflags, LPVOID lParam);

LONG WINAPI ChangeDisplaySettingsExA95(LPCSTR lpszDeviceName, DEVMODEA *lpDevMode, HWND hwnd, DWORD dwflags, LPVOID lParam)
{
	ChangeDisplaySettingsExA_func ChangeDisplaySettingsExA_proc = NULL;
	HANDLE u32 = GetModuleHandle("user32.dll");
	
	if(u32)
	{
		ChangeDisplaySettingsExA_proc = (ChangeDisplaySettingsExA_func)GetProcAddress(u32, "ChangeDisplaySettingsExA");
		if(ChangeDisplaySettingsExA_proc)
		{
			return ChangeDisplaySettingsExA_proc(lpszDeviceName, lpDevMode, hwnd, dwflags, lParam);
		}
	}
        ...
        UINT adapter_idx, const struct wined3d_display_mode *mode)
{
    ...
    DEVMODEA new_mode, current_mode;
    RECT clip_rc;
    LONG ret;
    ...
    if (mode)
    {
        ...
        else if(format->byte_count == 1) /* JHFIX: don't use 8bit mode, because palette usualy don't work  */
        {
        	new_mode.dmBitsPerPel = 4 * CHAR_BIT;
        }
        else
        ...
        if (mode->scanline_ordering != WINED3D_SCANLINE_ORDERING_UNKNOWN)
        {
            ...
            if (mode->scanline_ordering == WINED3D_SCANLINE_ORDERING_INTERLACED)
                DUMMYACCESS1(new_mode, u2, dmDisplayFlags) |= DM_INTERLACED;
        }
        new_format_id = mode->format_id;
    }
    else
    {
        if (!EnumDisplaySettingsA95(adapter->DeviceName, ENUM_REGISTRY_SETTINGS, &new_mode))
        {
    ...
    if (ret != DISP_CHANGE_SUCCESSFUL)
    {
        ...
        {
            WARN("ChangeDisplaySettingsExA failed, trying without the refresh rate.\n");
            new_mode.dmFields &= ~DM_DISPLAYFREQUENCY;
        ...
        UINT adapter_idx, DWORD flags, struct wined3d_adapter_identifier *identifier)
{
    ...
    if (identifier->device_name_size)
    {
        ...
        strcpy(identifier->device_name, adapter->DeviceName);
    }
...
static void wined3d_adapter_init_fb_cfgs(struct wined3d_adapter *adapter, HDC dc)
{
		...
		if (gl_info->supported[WGL_ARB_PIXEL_FORMAT])
    {
        ...
        adapter->cfgs = calloc(1, cfg_count * sizeof(*adapter->cfgs));
    ...
    else
    {
        ...
        do
        {
		        for (i = 0, adapter->cfg_count = 0; i < cfg_count; ++i)
		        {
		            ...
		            if (pfd.dwFlags & (PFD_GENERIC_FORMAT | PFD_GENERIC_ACCELERATED))
		            {
		            		if(sw_failback == 1)
		            		{
		            			...
		            			{
		            				WARN("Using format %d even it isn't ICD accelerated (FLAGS: %X).\n", format_id, pfd.dwFlags);
		            			}
		            		}
		            		else
		            		{
		            	    TRACE("Skipping format %d because it isn't ICD accelerated (%X).\n", format_id, pfd.dwFlags);
		            	    continue;
		            	  }
		            }
      	...
      	} while(adapter->cfg_count == 0);
    }
}

typedef BOOL (WINAPI *EnumDisplayDevicesA_func)(LPCSTR lpDevice, DWORD iDevNum, PDISPLAY_DEVICEA lpDisplayDevice, DWORD dwFlags);

BOOL WINAPI EnumDisplayDevicesA95(LPCSTR lpDevice, DWORD iDevNum, PDISPLAY_DEVICEA lpDisplayDevice, DWORD dwFlags)
{
	EnumDisplayDevicesA_func EnumDisplayDevicesA_proc = NULL;
	HANDLE u32 = GetModuleHandle("user32.dll");
	if(u32)
	{
		EnumDisplayDevicesA_proc = (EnumDisplayDevicesA_func)GetProcAddress(u32, "EnumDisplayDevicesA");
	}
	
	if(EnumDisplayDevicesA_proc)
	{
		return EnumDisplayDevicesA_proc(lpDevice, iDevNum, lpDisplayDevice, dwFlags);
	}
	...
	if(lpDisplayDevice)
	{
		strcpy(lpDisplayDevice->DeviceName, "\\\\.\\DISPLAY1");
		strcpy(lpDisplayDevice->DeviceString, "Wine adapter");
		lpDisplayDevice->StateFlags = DISPLAY_DEVICE_ACTIVE | DISPLAY_DEVICE_PRIMARY_DEVICE | DISPLAY_DEVICE_VGA_COMPATIBLE;
	}
...
# define USE_GL_FUNC(f) gl_info->gl_ops.gl.p_##f = (void *)GetProcAddress(mod_gl, #f);
        ALL_WGL_FUNCS
# undef USE_GL_FUNC
        /* JHFIX: there is something missing... completed! */
				gl_info->gl_ops.wgl.p_wglCopyContext         = (void *)GetProcAddress(mod_gl, "wglCopyContext");
    ...
    {
        FILETIME ftm;
        WARN("Failed to set adapter LUID (%#x).\n", GetLastError());
        //return FALSE;
        /* Windows 9x - if we can't allocate unique id, we fabricate any */
        GetSystemTimeAsFileTime(&ftm);
        
        /*adapter->luid.HighPart  = 0x00FFA34D;
        adapter->luid.LowPart   = 0xDD3DDEAD;*/
        adapter->luid.HighPart  = ftm.dwLowDateTime;
        adapter->luid.LowPart   = ftm.dwHighDateTime;
    }
...
#ifndef VBOX_WITH_WINE_FIX_POLYOFFSET_SCALE
    ...
    EnumDisplayDevicesA95(NULL, ordinal, &display_device, 0);
    WARN("DeviceName: %s\n", display_device.DeviceName);
    strcpy(adapter->DeviceName, display_device.DeviceName);
...
HRESULT wined3d_init(struct wined3d *wined3d, DWORD flags)
{
    ...
    TRACE("Initializing adapters - flags: %#x.\n", flags);
