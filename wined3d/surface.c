...
static HRESULT surface_create_dib_section(struct wined3d_surface *surface)
{
    ...
    switch (format->byte_count)
    {
        ...
        case 4:
            ...
            b_info = calloc(1, sizeof(BITMAPINFOHEADER) + 3 * sizeof(DWORD));
            break;

        case 3:
            b_info = calloc(1, sizeof(BITMAPINFOHEADER));
            break;
        ...
        DWORD dst_location)
{
    ...
    else
    {
        ...
        if (surface->flags & SFLAG_NONPOW2)
        {
            ...
            mem = malloc( src_pitch * surface->pow2Height);
        }
        else
        {
            mem = data.addr;
        ...
        struct wined3d_context *context, BOOL srgb)
{
    ...
    if (format.convert)
    {
        ...
        if (!(mem = malloc( dst_pitch * height)))
        {
            ERR("Out of memory (%u).\n", dst_pitch * height);
            context_release(context);
            return E_OUTOFMEMORY;
        ...
        const WINEDDBLTFX *fx, enum wined3d_texture_filter_type filter)
{
    ...
    struct wined3d_map_desc dst_map, src_map = {0};
