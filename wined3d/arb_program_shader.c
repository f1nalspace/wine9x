...
static void clone_sig(struct wined3d_shader_signature *new, const struct wined3d_shader_signature *sig)
{
    ...
    new->elements = malloc(sizeof(*new->elements) * new->element_count);
        ...
        const struct arb_ps_compile_args *args)
{
    ...
    if(shader_data->shader_array_size == shader_data->num_gl_shaders) {
        ...
        {
        	  size_t old_size = shader_data->shader_array_size;
            new_size = shader_data->shader_array_size + max(1, shader_data->shader_array_size / 2);
            //new_array = HeapReAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, shader_data->gl_shaders, new_size * sizeof(*shader_data->gl_shaders));
            new_array = realloc(shader_data->gl_shaders, // HEAP_ZERO_MEMORY
                                    new_size * sizeof(*shader_data->gl_shaders));
            if(new_array && new_size > old_size) {
                memset(new_array+old_size, 0, (new_size - old_size)*sizeof(*shader_data->gl_shaders));
            }
...
static HRESULT shader_arb_alloc(struct wined3d_device *device, const struct wined3d_vertex_pipe_ops *vertex_pipe,
        const struct fragment_pipeline *fragment_pipe)
{
    struct shader_arb_priv *priv = calloc(1, sizeof(*priv));
    ...
    priv->vshader_const_dirty = malloc(
            sizeof(*priv->vshader_const_dirty) * d3d_info->limits.vs_uniform_count);
    ...
    priv->pshader_const_dirty = malloc(
            sizeof(*priv->pshader_const_dirty) * d3d_info->limits.ps_uniform_count);
...
fail:
    free(priv->pshader_const_dirty);
    free(priv->vshader_const_dirty);
...
static void record_instruction(struct list *list, const struct wined3d_shader_instruction *ins)
{
    ...
    struct recorded_instruction *rec = calloc(1, sizeof(*rec));
    if(!rec)
    {
        ERR("Out of memory\n");
        return;
    ...
    rec->ins = *ins;
    dst_param = malloc(sizeof(*dst_param));
    if(!dst_param) goto free;
    *dst_param = *ins->dst;
    if (ins->dst->reg.idx[0].rel_addr)
    {
        rel_addr = malloc(sizeof(*rel_addr));
        if (!rel_addr)
            goto free;
    ...
    src_param = malloc(sizeof(*src_param) * ins->src_count);
    if (!src_param)
        goto free;
    for (i = 0; i < ins->src_count; ++i)
    {
...
static void fragment_prog_arbfp(struct wined3d_context *context, const struct wined3d_state *state, DWORD state_id)
{
    ...
    if (!use_pshader)
    {
        ...
        if(!desc) {
            struct arbfp_ffp_desc *new_desc = malloc(sizeof(*new_desc));
            if (!new_desc)
            {
                ERR("Out of memory\n");
                return;
